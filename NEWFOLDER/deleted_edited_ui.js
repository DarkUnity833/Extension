...
(() => {
        ...
        for (const el of candidates) {
            ...
            for (const msgEl of possible) {
                try {
                    ...
                    if (info?.cmid != null) {
                        return { peerId: info.peerId ?? getPeerId(), cmid: String(info.cmid), messageId: info.messageId ?? null, raw: info.raw || null, element: msgEl };
                    }
                } catch (_) {}
    ...
    function sanitizeAttachmentsFromMessage(msg) {
        const candidates = [
            msg?.attachments,
            msg?.message?.attachments,
            msg?.msg?.attachments,
            msg?.conversation_message?.attachments,
            msg?.media?.attachments
        ];
        let atts = candidates.find(Array.isArray) || [];
        if (!atts.length && msg?.chunks && Array.isArray(msg.chunks)) {
            atts = msg.chunks.filter(x => x && (x.attach || x.attachment)).map(x => x.attach || x.attachment);
        }

        // VK's current messenger keeps attachments of forwarded messages in
        // fwd_messages instead of the outer message.attachments array.  The
        // old downloader therefore saw the forwarded message itself, but not
        // its photos. Flatten only forwarded-message attachment arrays here;
        // keep the original attachment objects intact so photo IDs/access keys
        // survive and can be resolved through photos.getById().
        const forwarded = [];
        const seenForward = new Set();
        const walkForwarded = (node, depth = 0) => {
            if (!node || typeof node !== 'object' || depth > 12 || seenForward.has(node)) return;
            seenForward.add(node);

            for (const key of ['fwd_messages', 'forwarded_messages', 'forwardedMessages', 'forwards', 'forwarded']) {
                const list = node?.[key];
                if (!Array.isArray(list)) continue;
                for (const fm of list) {
                    if (!fm || typeof fm !== 'object') continue;
                    if (Array.isArray(fm.attachments)) forwarded.push(...fm.attachments);
                    walkForwarded(fm, depth + 1);
                }
            }
        };
        walkForwarded(msg);
        if (forwarded.length) atts = atts.concat(forwarded);

        // De-duplicate the flattened list. A forwarded attachment can be
        // exposed both in the message object and in its rendered/API wrapper.
        const seenAttachments = new Set();
        atts = atts.filter(att => {
            if (!att || typeof att !== 'object') return false;
            const t = String(att.type || '').toLowerCase();
            const media = att[t] && typeof att[t] === 'object' ? att[t] : att.photo || att.video || att.doc || att.audio_message;
            const owner = media?.owner_id ?? media?.ownerId ?? att.owner_id ?? att.ownerId ?? '';
            const id = media?.id ?? media?.photo_id ?? media?.video_id ?? att.id ?? '';
            const key = `${t}|${owner}|${id}|${att.access_key || media?.access_key || ''}`;
            if (seenAttachments.has(key)) return false;
            seenAttachments.add(key);
            return true;
        });

        // Keep full VK attachment structure. Video messages (circles) require
        // video.type=video_message and video.files to resolve the real mp4 URL.
        try {
            return JSON.parse(JSON.stringify(atts.map(att => {
                if (att?.type === 'video' && att.video) {
                    return {
                        type: 'video',
                        video: {
                            ...att.video,
                            files: att.video.files || {},
                            type: att.video.type || null,
                            id: att.video.id,
                            owner_id: att.video.owner_id,
                            external_id: att.video.external_id || null
                        }
                    };
                }
                return att;
    ...
    function makeAttachmentsMenuItem(menu) {
        if (!menu || !contextMessage) return;
        const actionMenu = actualActionMenu(menu);
        if (!actionMenu || !isMessageActionMenu(actionMenu) || actionMenu.querySelector('.vke-context-attachments-item')) return;
        ...
        item.classList.add('vke-context-attachments-item');
        item.removeAttribute('style');
        item.setAttribute('aria-label', 'Скачать вложения');
        item.setAttribute('tabindex', '0');
        item.type = 'button';
        item.innerHTML = '<i class=\"ActionsMenuAction__icon\"><span aria-hidden=\"true\">⬇️</span></i><span class=\"ActionsMenuAction__title\">Скачать вложения</span>';
        const activate = async ev => {
            ev?.preventDefault?.(); ev?.stopPropagation?.(); ev?.stopImmediatePropagation?.();
            clearHistoryMenuTimers();
            item.setAttribute('aria-disabled', 'true');
            item.classList.add('vke-context-attachments-loading');
            const raw = contextMessage?.raw || contextMessage;
            let atts = sanitizeAttachmentsFromMessage(raw);

            // findMessageContext intentionally stores only IDs. Current VK keeps
            // attachments in the React message object/API response, so refresh
            // the exact message by cmid before giving up.
            if (!atts.length && contextMessage?.peerId != null && contextMessage?.cmid != null) {
                try {
                    const r = await bridgeCall('API_CALL', {
                        method: 'messages.getByConversationMessageId',
                        params: {
                            peer_id: contextMessage.peerId,
                            conversation_message_ids: String(contextMessage.cmid),
                            extended: 0
                        }
                    });
                    const api = r?.success ? r.response : r;
                    const msg = api?.items?.[0] || api?.response?.items?.[0] || null;
                    atts = sanitizeAttachmentsFromMessage(msg);
                    if (atts.length) contextMessage.raw = msg;
                } catch (_) {}
            }

            if (!atts.length) {
                item.removeAttribute('aria-disabled');
                item.classList.remove('vke-context-attachments-loading');
                return;
            }

            const requestId = `att_${Date.now()}_${Math.random().toString(36).slice(2,7)}`;
            // MAIN and ISOLATED worlds do not share JS object state. Store a tiny
            // DOM marker on the exact message row as the cross-world bridge so the
            // attachment downloader can resolve the exact <video> instead of
            // accidentally picking another circle from the conversation.
            const domKey = `vke_att_${Date.now()}_${Math.random().toString(36).slice(2,9)}`;
            try {
                const el = contextMessage?.element;
                if (el?.setAttribute) {
                    el.setAttribute('data-vke-attachment-context', domKey);
                    setTimeout(() => {
                        try {
                            if (el.getAttribute('data-vke-attachment-context') === domKey)
                                el.removeAttribute('data-vke-attachment-context');
                        } catch (_) {}
                    }, 60000);
                }
            } catch (_) {}
            window.postMessage({
                source:'vke-main',
                type:'VKE_DOWNLOAD_MESSAGE_ATTACHMENTS',
                requestId,
                domKey,
                peerId: contextMessage?.peerId ?? getPeerId(),
                cmid: contextMessage?.cmid ?? null,
                attachments: atts
            }, '*');
            item.removeAttribute('aria-disabled');
            item.classList.remove('vke-context-attachments-loading');
        ...
        const historyItem = actionMenu.querySelector('.vke-context-history-item');
        if (historyItem?.parentNode === actionMenu) actionMenu.insertBefore(item, historyItem.nextSibling);
