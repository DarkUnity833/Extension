...
.vke-media-quality-menu.vke-visible{display:flex!important}.vke-media-quality-menu{display:none!important;position:fixed!important;min-width:154px!important;max-width:240px!important;left:0;top:0;right:auto;bottom:auto;transform:none;pointer-events:auto!important;padding:6px!important;border-radius:12px!important;background:var(--vkui--color_background_modal,#fff)!important;color:var(--vkui--color_text_primary,#000)!important;box-shadow:0 8px 30px rgba(0,0,0,.28)!important;border:1px solid var(--vkui--color_separator_primary_alpha,rgba(0,0,0,.10))!important;z-index:2147483647!important;flex-direction:column!important;gap:2px!important}.vke-media-quality-menu.vke-vkvideo-dark{background:#222222!important;color:#fff!important;border-color:rgba(255,255,255,.12)!important;box-shadow:0 10px 34px rgba(0,0,0,.55)!important}.vke-media-quality-menu.vke-vkvideo-dark .vke-media-quality-item:hover{background:rgba(255,255,255,.10)!important}.vke-media-quality-menu.vke-vkvideo-dark .vke-media-quality-sub{color:rgba(255,255,255,.70)!important}.vke-media-quality-menu.vke-vkvideo-dark .vke-media-quality-item{color:#ffffff!important}.vke-video-quality-menu{opacity:0!important;transform:translateY(5px) scale(.97)!important;transform-origin:center bottom!important;transition:opacity .12s ease,transform .12s ease!important}.vke-video-quality-menu.vke-visible{opacity:1!important;transform:translateY(0) scale(1)!important}
.vke-media-quality-menu.vke-visible{display:flex!important}
.vke-media-quality-title{padding:5px 9px 4px;font:600 12px/16px -apple-system,BlinkMacSystemFont,Roboto,sans-serif;color:#fff!important;opacity:.82}.vke-media-quality-menu.vke-vkvideo-dark .vke-media-quality-title{color:#ffffff!important;opacity:.92}
...
@keyframes vke-media-spin{to{transform:rotate(360deg)}}`;
  ...
  function isVkVideoHost(){return /(^|\.)vkvideo\.(ru|com)$/i.test(location.hostname)}
  ...
  function detectVkVideoDark(){
    // Theme must follow the actual VK Video page, not the extension popup.
    // VK changes theme through several different roots/attributes depending on
    // the player/page version, so inspect the common theme markers first and
    // then fall back to computed page colors.
    const nodes=[
      document.documentElement,
      document.body,
      document.querySelector('[data-testid="video-page"]'),
      document.querySelector('[data-testid="video-page-info"]'),
      document.querySelector('#root'),
      document.querySelector('[class*="VideoPage" i]')
    ].filter(Boolean);
    for(const n of nodes){
      const attrs=[
        n.getAttribute?.('data-theme'),
        n.getAttribute?.('data-color-scheme'),
        n.getAttribute?.('data-scheme'),
        n.getAttribute?.('theme'),
        n.getAttribute?.('color-scheme'),
        n.className
      ].join(' ').toLowerCase();
      if(/(?:^|[\s_-])(dark|night)(?:$|[\s_-])/.test(attrs) || /theme[^a-z]*(dark|night)/.test(attrs)) return true;
      if(/(?:^|[\s_-])(light|day)(?:$|[\s_-])/.test(attrs) || /theme[^a-z]*(light|day)/.test(attrs)) return false;
      try{
        ...
        for(const prop of ['--vkui--color_background_content','--vkui--color_background_modal','--vkui--color_background','--vk-video-background']){
          const l=luminanceFromColor(cs.getPropertyValue(prop));
          if(l!=null){if(l<110)return true;if(l>190)return false;}
        }
      }catch{}
    }
    try{
      const candidates=[document.body,document.documentElement,...document.querySelectorAll('main,[role="main"],[class*="Root" i],[class*="Page" i]')];
      let darkScore=0, lightScore=0;
      for(const n of candidates.slice(0,30)){
        const cs=getComputedStyle(n);
        const bg=luminanceFromColor(cs.backgroundColor);
        const fg=luminanceFromColor(cs.color);
        if(bg!=null){if(bg<100)darkScore+=3; else if(bg>200)lightScore+=3;}
        if(fg!=null){if(fg>180)darkScore++; else if(fg<90)lightScore++;}
      }
      if(darkScore>lightScore)return true;
      if(lightScore>darkScore)return false;
    }catch{}
    // Respect the browser/site preference as a last resort.
    try{return !!window.matchMedia?.('(prefers-color-scheme: dark)').matches}catch{return false}
  }
  function syncMenuTheme(menu){
    if(!menu) return;
    const darkHost=isVkVideoHost();
    menu.classList.toggle('vke-vkvideo-dark',darkHost);
    if(darkHost){
      // vkvideo.ru: force the quality-menu surface to the requested dark gray.
      // Use an inline !important style because VK can override extension CSS
      // variables/selectors on this page.
  ...
  function startThemeObserver(){
    ...
    try{
      const obs=new MutationObserver(()=>{ if(globalMenu) syncMenuTheme(globalMenu); });
      obs.observe(document.documentElement,{attributes:true,attributeFilter:['class','data-theme','data-color-scheme','data-scheme','style'],subtree:true});
    }catch{}
    try{window.matchMedia?.('(prefers-color-scheme: dark)').addEventListener?.('change',()=>{if(globalMenu)syncMenuTheme(globalMenu)})}catch{}
  }
  function getMenu(w){
    startThemeObserver();
    if(!globalMenu){
  ...
  function closeMenu(w){
    ...
    if(globalMenuOwner?._menuPositionTimer){clearInterval(globalMenuOwner._menuPositionTimer);globalMenuOwner._menuPositionTimer=null}
  ...
  function positionMenu(w,menu){
    if(!w||!menu||!document.documentElement.contains(menu))return;
    if(menu.dataset.vkeKind==='video') return positionVideoMenu(w,menu);
    const btn=w.querySelector('.vke-media-dl-btn')||w;
    const r=btn.getBoundingClientRect();
    if(!r||(!r.width&&!r.height)||!Number.isFinite(r.left))return;

    // The menu is a single document-level popup. Always recompute its screen
    // coordinates from the actual button rect; never rely on the wrapper's
    // layout/positioning because VK frequently reparents/rebuilds controls.
    menu.style.position='fixed';
    menu.style.right='auto';
    menu.style.bottom='auto';
    menu.style.transform='none';

    const mw=Math.max(158,Math.min(260,menu.scrollWidth||menu.offsetWidth||158));
    const mh=Math.max(40,menu.scrollHeight||menu.offsetHeight||40);
    ...
    // Prefer above the download button. If there is not enough room, put it
    // below it; finally clamp inside the viewport.
  ...
  function positionVideoMenu(w,menu){
    if(!w||!menu)return;
    // Portal the single menu into body and position it from the real button's
    // document coordinates. This avoids VK's transformed player containers
    // breaking position:fixed and placing the menu at 0,0.
    if(menu.parentNode!==document.body){
      try{document.body.appendChild(menu)}catch{}
    }
    const btn=w.querySelector?.('.vke-media-dl-btn')||w;
    let r=btn.getBoundingClientRect?.();
    // VK occasionally replaces the inner button while our wrapper survives.
    // Fall back to the wrapper/actual button geometry instead of ever using
    // (0,0) when the first rect is stale or empty.
    if(!r || (r.width<2&&r.height<2) || !Number.isFinite(r.left)) r=w.getBoundingClientRect?.();
    if(!r || (r.width<2&&r.height<2) || !Number.isFinite(r.left)) return;
    menu.style.position='fixed';
    menu.style.right='auto'; menu.style.bottom='auto';
    menu.style.visibility='hidden';
    // The menu is a direct child of body, so getBoundingClientRect() already
    // returns viewport coordinates. Do not add page scroll offsets.
    const mw=Math.max(154,Math.min(300,menu.offsetWidth||menu.scrollWidth||154));
    const mh=Math.max(40,menu.offsetHeight||menu.scrollHeight||40);
    const gap=8; const vw=Math.max(1,document.documentElement.clientWidth||window.innerWidth||1);
    let left=r.left+(r.width-mw)/2;
    left=Math.max(gap,Math.min(left,vw-mw-gap));
    let top=r.top-mh-gap;
    if(top<gap) top=r.bottom+gap;
    menu.style.left=`${Math.round(left)}px`;
    menu.style.top=`${Math.round(top)}px`;
    menu.style.transform='translateY(5px) scale(.97)';
    menu.style.visibility='visible';
    requestAnimationFrame(()=>{
      if(globalMenuOwner===w&&menu.classList.contains('vke-visible')){
  ...
  function buildMenu(w,btn,kind,info,status='') {
    ...
    menu.className=`vke-media-quality-menu vke-visible${kind==='video'?' vke-video-quality-menu':''}`;
    menu.dataset.vkeKind=kind;
    syncMenuTheme(menu);
    ...
    if(status){
      ...
      requestAnimationFrame(()=>{ syncMenuTheme(menu); positionMenu(w,menu); });
      return;
    }
    if(kind==='clip'){
    ...
    clearInterval(w._menuPositionTimer);
    w._menuPositionTimer=setInterval(()=>{
      if(globalMenuOwner!==w || !globalMenu?.classList.contains('vke-visible')){
        clearInterval(w._menuPositionTimer);
        w._menuPositionTimer=null;
        return;
      }
      syncMenuTheme(globalMenu);
      positionMenu(w,globalMenu);
    },120);
  }
  ...
  const uiResolveInflight = new Map();
  window.addEventListener('vke-video-enriched',ev=>{
    const d=ev.detail||{};
    const owner=globalMenuOwner;
    if(!owner||!d.id||owner.dataset.vkeVideoId!==d.id)return;
    if(!(owner.matches(':hover')||globalMenu?.matches(':hover')))return;
    const b=owner.querySelector?.('.vke-media-dl-btn');
    if(b) buildMenu(owner,b,'video',d.info||null);
  });
  let currentInfoLocal=()=>null;
  try{ if(window.__vkeMediaCore?.currentInfo) currentInfoLocal=window.__vkeMediaCore.currentInfo; }catch{}

  async function resolveVideoFastUI(id,clip=false,root=null,accessKey=null){
    const normalized=(id&&String(id).match(/-?\d+_\d+/)?.[0])||id;
    if(!normalized) throw new Error('ID видео/клипа не найден');
    const key=`video:${normalized}`;
    const cached=coreCache.get(key)?.info;
    // Never make the user wait for the complete resolver once a usable result
    // is already known. A background pass continues collecting the full set.
    if(cached?.video?.qualities?.length){
      if(!uiResolveInflight.has(normalized)){
        const job=Promise.resolve().then(()=>window.__vkeMediaCore?.resolveVideo?.(normalized,clip,root,accessKey)).then(info=>{
          if(info?.qualities?.length){
            const merged={...info,id:normalized,video:{id:normalized,files:info.files||{},qualities:info.qualities}};
            coreCache.set(key,{ts:Date.now(),info:merged});
            window.dispatchEvent(new CustomEvent('vke-video-enriched',{detail:{id:normalized,info:merged}}));
          }
          return info;
        }).catch(()=>null).finally(()=>uiResolveInflight.delete(normalized));
        uiResolveInflight.set(normalized,job);
      }
      return cached;
    }

    // Fast path: the active player source is immediately usable. The expensive
    // API/network resolver is deliberately started in the background.
    let live=null;
    try{ live=currentInfoLocal('video'); }catch{}
    const src=live?.src||'';
    const q=Number(live?.height)||0;
    if(src&&/^https?:\/\//i.test(src)&&!/^blob:/i.test(src)){
      const fast={id:normalized,src,height:q,video:{id:normalized,files:{},qualities:[{url:src,q:q||720,key:'live'}]},fallback:[]};
      coreCache.set(key,{ts:Date.now(),info:fast});
      if(!uiResolveInflight.has(normalized)){
        const job=Promise.resolve().then(()=>window.__vkeMediaCore?.resolveVideo?.(normalized,clip,root,accessKey)).then(info=>{
          if(info?.qualities?.length){
            { const merged={...info,id:normalized,video:{id:normalized,files:info.files||{},qualities:info.qualities}}; coreCache.set(key,{ts:Date.now(),info:merged}); window.dispatchEvent(new CustomEvent('vke-video-enriched',{detail:{id:normalized,info:merged}})); }
          }
        }).catch(()=>{}).finally(()=>uiResolveInflight.delete(normalized));
        uiResolveInflight.set(normalized,job);
      }
      return fast;
    }
    return resolveCached('video',false,normalized);
  }
  ...
  function make(kind,title,targetVideo=null){
    ...
    if(kind==='video') {
      const openAndResolve=()=>{
        ...
        resolveVideoFastUI(exactId || location.href,false,null,null).then(info=>{
          if(info?.video?.qualities?.length){ uiGoodCache.set(key,{ts:Date.now(),info}); }
