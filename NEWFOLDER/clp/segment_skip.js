...
(() => {
  ...
  function getTranscriptPlayerRoot(v){
    if(!v)return null;
    let n=v;
    for(let i=0;i<40&&n;i++){
      try{
        if(n.classList?.contains('root-container')) return n;
        ...
        n=host||p||n.parentNode||null;
      }catch{n=n?.parentElement||n?.parentNode||null;}
    }
    try{
      const host=v.closest?.('vk-video-player');
      const r=host?.shadowRoot?.querySelector?.('.root-container')||host?.querySelector?.('.root-container');
      if(r)return r;
    }catch{}
    return v.parentElement||null;
  }

  async function waitForVKTranscriptTracks(v, timeoutMs=15000){
    if(!v)return [];
    const deadline=Date.now()+timeoutMs;
    while(Date.now()<deadline){
      if(current.video!==v)return [];
      try{
        const list=v.textTracks;
        if(list && list.length){
          const tracks=Array.from(list).filter(t=>!t.kind||t.kind==='subtitles'||t.kind==='captions');
  ...
  async function loadTranscriptFromVideo(v){
    ...
    const playerRoot=getTranscriptPlayerRoot(v);
    const rootVideo=playerRoot?.querySelector?.('video') || v;
    const firstTracks=await waitForVKTranscriptTracks(rootVideo,15000);
    if(firstTracks.length){
      emitTranscriptDebug(`TextTracks обнаружены: ${firstTracks.length}`);
    }else{
      emitTranscriptDebug('TextTracks пока отсутствуют; продолжаю ожидание/поиск ресурсов');
    }
    const initialCandidates=transcriptCandidates(rootVideo).slice(0,12);
    const candidates=[rootVideo,v,...initialCandidates.filter(x=>x!==rootVideo&&x!==v)];
  ...
  async function loadTranscript(){
    ...
    transcriptPromise=(async()=>{
      try{
        ...
        // One more delayed resource scan: VK may attach the track/network resource after the initial 20s window.
        await new Promise(r=>setTimeout(r,1200));
        const delayed=await loadTranscriptFromVideo(current.video);
        if(current.id===initialId && delayed.length){
          publishTranscript(delayed,expectedId,'delayed-loader');
          return true;
        }
  ...
  function findPlayerControls(v){
    if(!v) return null;
    // First search inside the video's current root (including shadow DOM).
    try{
      const root=v.getRootNode?.();
      const own=root?.querySelector?.('[data-testid="player_controls"], .controls');
      if(own)return own;
    }catch{}
    // Walk through shadow hosts and inspect each root.
    let node=v;
    for(let i=0;i<40&&node;i++){
      try{
        const p=node.parentElement;
        const c=p?.querySelector?.('[data-testid="player_controls"]');
        if(c)return c;
        const host=node.getRootNode?.()?.host;
        if(host){
          const c2=host.shadowRoot?.querySelector?.('[data-testid="player_controls"]');
          if(c2)return c2;
          node=host;
        }else node=p||node.parentNode||null;
      }catch{node=node?.parentElement||node?.parentNode||null;}
    }
    for(const r of reachableRoots()){
      try{const c=r.querySelector?.('[data-testid="player_controls"]');if(c)return c;}catch{}
    }
    return null;
  }

  function injectTranscriptSearchButton(v){
    const controls=findPlayerControls(v);
    if(!controls) return;
    const root=controls.getRootNode?.()||controls;
    ensureSearchShadowStyle(root);

    let left=controls.querySelector?.('.controls-left,[class*="controls-left"]');
    if(!left){
      left=controls.querySelector?.('[data-testid="time"]')?.parentElement||controls.firstElementChild;
    }
    if(!left) return;

    const time=controls.querySelector?.('[data-testid="time"]')||left.querySelector?.('[data-testid="time"]');
    if(!time) return;

    let slot=left.querySelector?.('#vke-transcript-search-slot-v1');
    let btn=slot?.querySelector?.('#vke-transcript-search-button-v1');
    if(!slot){
      slot=document.createElement('div');
      slot.id='vke-transcript-search-slot-v1';
      slot.className='btn-container vke-transcript-search-slot';
      slot.setAttribute('data-vke-control','transcript-search');
      slot.style.cssText='display:flex!important;align-items:center!important;flex:0 1 185px!important;width:185px!important;min-width:120px!important;height:40px!important;overflow:visible!important;position:relative!important;z-index:200!important;';
    }
    if(!btn){
      const tooltip=document.createElement('div');
      tooltip.className='tooltip-wrapper s-23 full-width';
      btn=document.createElement('button');
      btn.type='button';
      btn.id='vke-transcript-search-button-v1';
      btn.className='vke-transcript-search-button';
      btn.setAttribute('aria-label','Поиск по словам в тексте');
      btn.title='Поиск по словам в тексте';
      btn.innerHTML='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M10.5 3a7.5 7.5 0 1 0 4.69 13.36l4.72 4.72a.9.9 0 0 0 1.27-1.27l-4.72-4.72A7.5 7.5 0 0 0 10.5 3Zm0 1.8a5.7 5.7 0 1 1 0 11.4 5.7 5.7 0 0 1 0-11.4Z"></path></svg><span class="vke-transcript-search-label">Поиск по словам в тексте</span>';
      btn.addEventListener('pointerdown',e=>{e.preventDefault();e.stopPropagation();},true);
      btn.addEventListener('mousedown',e=>{e.preventDefault();e.stopPropagation();},true);
      btn.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();e.stopImmediatePropagation();openTranscriptSearch(btn);},true);
      tooltip.appendChild(btn); slot.appendChild(tooltip);
    }
    if(!slot.isConnected){
      // The VK player may rerender its controls wholesale. Reattach our control.
      const directNext=time.nextElementSibling;
      if(directNext && directNext.classList?.contains('btn-container') && !directNext.querySelector?.('#vke-transcript-search-button-v1')){
        directNext.before(slot);
      }else{
        time.parentNode?.insertBefore(slot,time.nextSibling);
      }
    } else if(slot.previousElementSibling!==time){
      time.parentNode?.insertBefore(slot,time.nextSibling);
    }
    // Never let VK's flex compression reduce the control to zero.
    try{
      const lp=getComputedStyle(left);
      if(lp.overflow==='hidden') left.style.setProperty('overflow','visible','important');
      left.style.setProperty('min-width','0','important');
    }catch{}
    updateTranscriptSearchButtonState(btn);
  }

  function ensureSearchShadowStyle(root){
    if(!root?.querySelector)return;
    if(root.querySelector('style[data-vke-transcript-search-style]'))return;
    const st=document.createElement('style');
    st.setAttribute('data-vke-transcript-search-style','1');
    st.textContent=`
      #vke-transcript-search-slot-v1{display:flex!important;align-items:center!important;flex:0 0 185px!important;width:185px!important;height:40px!important;min-width:185px!important;overflow:visible!important;position:relative!important;z-index:200!important}
      #vke-transcript-search-button-v1{display:inline-flex!important;align-items:center!important;justify-content:flex-start!important;gap:7px!important;width:100%!important;max-width:185px!important;min-width:120px!important;height:32px!important;min-height:32px!important;padding:0 10px!important;margin:0 4px!important;border:1px solid rgba(255,255,255,.12)!important;border-radius:8px!important;background:rgba(255,255,255,.07)!important;color:#fff!important;cursor:pointer!important;box-sizing:border-box!important;font:500 12px/32px -apple-system,BlinkMacSystemFont,Roboto,sans-serif!important;white-space:nowrap!important;overflow:hidden!important;text-overflow:ellipsis!important;opacity:.9!important;pointer-events:auto!important;position:relative!important;z-index:50!important}
      #vke-transcript-search-button-v1 svg{width:16px!important;height:16px!important;flex:0 0 16px!important;fill:currentColor!important;opacity:.75!important}
      #vke-transcript-search-button-v1 .vke-transcript-search-label{overflow:hidden!important;text-overflow:ellipsis!important;white-space:nowrap!important}
      #vke-transcript-search-button-v1:hover{background:rgba(255,255,255,.13)!important;opacity:1!important}
      #vke-transcript-search-button-v1.vke-transcript-ready{border-color:rgba(74,163,255,.3)!important;background:rgba(74,163,255,.12)!important}
      #vke-transcript-search-button-v1.vke-transcript-loading{opacity:.72!important}
    `;
    root.appendChild(st);
  }

  function updateTranscriptSearchButtonState(btn){
    if(!btn)return;
    const n=current.transcript?.length||0;
    btn.classList.toggle('vke-transcript-ready',n>0);
    btn.classList.toggle('vke-transcript-loading',!n);
    btn.setAttribute('data-cues',String(n));
  }
  ...
  function panelBase(){
    ...
    p.setAttribute('aria-label','Сегменты видео');
    p.style.position='fixed';
    p.style.zIndex='2147483647';
    p.style.visibility='hidden';
    p.style.pointerEvents='auto';
    document.body.appendChild(p);
    requestAnimationFrame(()=>{ try{p.style.visibility='visible';}catch{} });
    return p;
  }
  ...
  function transcriptBootstrapStep(){
    ...
    if(v && v!==transcriptBootstrapVideo){
      ...
      current.video=v;
      current.id=extractId(v) || (location.href.match(/video(-?\d+_\d+)/i)?.[1]||null) || current.id || extractId(document.body);
  ...
  function tick(){
    ...
    if(v!==current.video){
      ...
      current.transcriptAdHints=[];
      resetRunner();
      if(v){injectButton(v);injectTranscriptSearchButton(v);setTimeout(()=>injectTranscriptSearchButton(v),100);setTimeout(()=>injectTranscriptSearchButton(v),500);renderTimeline();
      ...
      setTimeout(()=>injectTranscriptSearchButton(v),120);
  ...
  css.textContent=`
    ...
    [data-testid="player_controls"]{position:relative!important;z-index:100!important;overflow:visible!important;}
    ...
    html body{scroll-padding-top:56px!important;}
    [data-testid="video_spa_header"],.VideoHeaderLayout__container--6Emcb{position:sticky!important;top:0!important;left:0!important;right:0!important;z-index:2147483647!important;isolation:isolate!important;transform:translateZ(0)!important;}
    [data-testid="video_spa_header"],.VideoHeaderLayout__container--6Emcb{background:var(--vkui--color_background_content,#222)!important;}
    [data-testid="video_spa_header"]::after,.VideoHeaderLayout__container--6Emcb::after{content:""!important;position:absolute!important;inset:0!important;z-index:-1!important;background:var(--vkui--color_background_content,#222)!important;}
    .vke-header-shield-v1{position:fixed!important;top:0!important;left:0!important;right:0!important;height:56px!important;z-index:2147483646!important;pointer-events:auto!important;background:var(--vkui--color_background_content,#222)!important;}
    [data-testid="video_spa_header"] *,.VideoHeaderLayout__container--6Emcb *{position:relative;z-index:auto;}
    body:not(:fullscreen) [data-testid="video_spa_header"]{margin-bottom:0!important;}
    body:not(:fullscreen) [data-testid="video_spa_header"] ~ *{ }
  ...
  try{
    const installHeaderShield=()=>{
      const h=document.querySelector('[data-testid="video_spa_header"],.VideoHeaderLayout__container--6Emcb');
      if(!h)return;
      if(!document.getElementById('vke-header-shield-v1')){
        const sh=document.createElement('div');
        sh.id='vke-header-shield-v1';
        sh.className='vke-header-shield-v1';
    ...
    installHeaderShield();
    setInterval(installHeaderShield,600);
  }catch{}
