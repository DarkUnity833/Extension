...
(() => {
  ...
  async function resolveVideo(id, clip = false, root = null, accessKey = null) {
    ...
    if (cached && Date.now() - cached.ts < 20000 && cached.data?.qualities?.length) return cached.data;
    ...
    const videoKey = key ? `${normalized}_${key}` : normalized;
    ...
    try { return await promise; } finally { inFlight.delete(normalized); }
  }
